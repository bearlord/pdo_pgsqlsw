...
{
	php_stream *stm;
	struct pdo_pgsqlsw_lob_self *self = ecalloc(1, sizeof(*self));
...
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	pdo_pgsqlsw_stmt *S = ecalloc(1, sizeof(pdo_pgsqlsw_stmt));
	int scrollable;
	int ret;
	char *nsql = NULL;
	size_t nsql_len = 0;
	...
	if (scrollable) {
		...
		if (pdo_attr_lval(driver_options, PDO_PGSQLSW_ATTR_DISABLE_PREPARES, H->disable_prepares) == 1) {
			execute_only = 1;
		}
	} else {
...
static PHP_METHOD(PDO, pgsqlLOBOpen)
{
	...
	if (lfd >= 0) {
		php_stream *stream = pdo_pgsqlsw_create_lob_stream(getThis(), lfd, oid);
		if (stream) {
