...
static char * _pdo_pgsqlsw_trim_message(const char *message, int persistent)
{
	...
	++i;
	tmp = (char *) pemalloc(i + 1, persistent);
...
static zend_string* _pdo_pgsqlsw_escape_credentials(const char *str)
{
    char *what = new char[20];
    strcpy(what, "\\'");
	if (str) {
		return (zend_string *) php_addcslashes_str(str, strlen(str), what, sizeof("\\'"));
	}

	return NULL;
}

int _pdo_pgsqlsw_error(pdo_dbh_t *dbh, pdo_stmt_t *stmt, int errcode, const char *sqlstate, const char *msg, const char *file, int line) /* {{{ */
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	pdo_error_type *pdo_err = stmt ? &stmt->error_code : &dbh->error_code;
	pdo_pgsqlsw_error_info *einfo = &H->einfo;
	...
	if (msg) {
		...
		einfo->errmsg = _pdo_pgsqlsw_trim_message(errmsg, dbh->is_persistent);
	}
...
static void _pdo_pgsqlsw_notice(pdo_dbh_t *dbh, const char *message) /* {{{ */
{
/*	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data; */
}
/* }}} */

static int pdo_pgsqlsw_fetch_error_func(pdo_dbh_t *dbh, pdo_stmt_t *stmt, zval *info) /* {{{ */
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	pdo_pgsqlsw_error_info *einfo = &H->einfo;
...
static int pgsqlsw_lob_close(php_stream *stream, int close_handle)
{
	struct pdo_pgsqlsw_lob_self *self = (struct pdo_pgsqlsw_lob_self*)stream->abstract;
...
static int pgsqlsw_lob_flush(php_stream *stream)
{
	return 0;
}

static int pgsqlsw_lob_seek(php_stream *stream, zend_off_t offset, int whence,
...
const php_stream_ops pdo_pgsqlsw_lob_stream_ops = {
	pgsqlsw_lob_write,
	pgsqlsw_lob_read,
	pgsqlsw_lob_close,
	pgsqlsw_lob_flush,
	"pdo_pgsqlsw lob stream",
	pgsqlsw_lob_seek,
	NULL,
	NULL,
	NULL
...
php_stream *pdo_pgsqlsw_create_lob_stream(zval *dbh, int lfd, Oid oid)
{
	php_stream *stm;
	struct pdo_pgsqlsw_lob_self *self = (struct pdo_pgsqlsw_lob_self*) ecalloc(1, sizeof(*self));
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)(Z_PDO_DBH_P(dbh))->driver_data;
	...
	stm = php_stream_alloc(&pdo_pgsqlsw_lob_stream_ops, self, 0, "r+b");
...
static int pgsqlsw_handle_closer(pdo_dbh_t *dbh) /* {{{ */
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	if (H) {
		if (H->server) {
...
static int pgsqlsw_handle_preparer(pdo_dbh_t *dbh, const char *sql, size_t sql_len, pdo_stmt_t *stmt, zval *driver_options)
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt *) ecalloc(1, sizeof(pdo_pgsqlsw_stmt));
	int scrollable;
	int ret;
	char *nsql = NULL;
	size_t nsql_len = 0;
	...
	S->H = H;
	stmt->driver_data = S;
	stmt->methods = &pgsqlsw_stmt_methods;
	...
	if (scrollable) {
		...
		if (pdo_attr_lval(driver_options, PDO_ATTR_DRIVER_SPECIFIC, H->disable_prepares) == 1) {
			execute_only = 1;
		}
	} else {
...
static zend_long pgsqlsw_handle_doer(pdo_dbh_t *dbh, const char *sql, size_t sql_len)
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	PGresult *res;
	zend_long ret = 1;
	...
	if (!(res = PQexec(H->server, sql))) {
		/* fatal error */
		pdo_pgsqlsw_error(dbh, PGRES_FATAL_ERROR, NULL);
		return -1;
	}
	qs = PQresultStatus(res);
	if (qs != PGRES_COMMAND_OK && qs != PGRES_TUPLES_OK) {
		pdo_pgsqlsw_error(dbh, qs, pdo_pgsqlsw_sqlstate(res));
		PQclear(res);
		return -1;
	}
...
static int pgsqlsw_handle_quoter(pdo_dbh_t *dbh, const char *unquoted, size_t unquotedlen, char **quoted, size_t *quotedlen, enum pdo_param_type paramtype)
{
...
static char *pdo_pgsqlsw_last_insert_id(pdo_dbh_t *dbh, const char *name, size_t *len)
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	char *id = NULL;
	PGresult *res;
	...
	if (res && (status == PGRES_TUPLES_OK)) {
		...
		*len = PQgetlength(res, 0, 0);
		...
		pdo_pgsqlsw_error(dbh, status, pdo_pgsqlsw_sqlstate(res));
	}
...
static int pdo_pgsqlsw_get_attribute(pdo_dbh_t *dbh, zend_long attr, zval *return_value)
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	...
	switch (attr) {
		...
		case PDO_PGSQLSW_ATTR_DISABLE_PREPARES:
...
/* {{{ */
static int pdo_pgsqlsw_check_liveness(pdo_dbh_t *dbh)
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
...
static int pgsqlsw_handle_in_transaction(pdo_dbh_t *dbh)
{
	pdo_pgsqlsw_db_handle *H;

	H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;

	return PQtransactionStatus(H->server) > PQTRANS_IDLE;
}

static int pdo_pgsqlsw_transaction_cmd(const char *cmd, pdo_dbh_t *dbh)
{
	pdo_pgsqlsw_db_handle *H = (pdo_pgsqlsw_db_handle *)dbh->driver_data;
	PGresult *res;
	int ret = 1;
	...
	if (PQresultStatus(res) != PGRES_COMMAND_OK) {
		pdo_pgsqlsw_error(dbh, PQresultStatus(res), pdo_pgsqlsw_sqlstate(res));
		ret = 0;
	}
...
static int pgsqlsw_handle_begin(pdo_dbh_t *dbh)
{
	return pdo_pgsqlsw_transaction_cmd("BEGIN", dbh);
}

static int pgsqlsw_handle_commit(pdo_dbh_t *dbh)
{
	int ret = pdo_pgsqlsw_transaction_cmd("COMMIT", dbh);

	/* When deferred constraints are used the commit could
	   fail, and a ROLLBACK implicitly ran. See bug #67462 */
	if (!ret) {
		dbh->in_txn = pgsqlsw_handle_in_transaction(dbh);
	}

	return ret;
}

static int pgsqlsw_handle_rollback(pdo_dbh_t *dbh)
{
	return pdo_pgsqlsw_transaction_cmd("ROLLBACK", dbh);
}
	...
	if (status == PGRES_COPY_IN && pgsql_result) {
		...
		ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(pg_rows), tmp) {
			...
			if (buffer_len < Z_STRLEN_P(tmp)) {
				...
				query = (char *) erealloc(query, buffer_len + 2); /* room for \n\0 */
			}
		...
		while ((pgsql_result = PQgetResult(H->server))) {
			if (PGRES_COMMAND_OK != PQresultStatus(pgsql_result)) {
				pdo_pgsqlsw_error(dbh, PGRES_FATAL_ERROR, pdo_pgsqlsw_sqlstate(pgsql_result));
				command_failed = 1;
			}
			PQclear(pgsql_result);
		}
...
static PHP_METHOD(PDO, pgsqlCopyFromFile)
{
	...
	if (!stream) {
		pdo_pgsqlsw_error_msg(dbh, PGRES_FATAL_ERROR, "Unable to open the file");
		PDO_HANDLE_DBH_ERR();
		RETURN_FALSE;
	}
...
static PHP_METHOD(PDO, pgsqlCopyToFile)
{
	...
	if (!stream) {
		pdo_pgsqlsw_error_msg(dbh, PGRES_FATAL_ERROR, "Unable to open the file for writing");
		PDO_HANDLE_DBH_ERR();
		RETURN_FALSE;
	}
	...
	if (status == PGRES_COPY_OUT && pgsql_result) {
		...
		while (1) {
			...
			if (ret == -1) {
				...
				if ((size_t)php_stream_write(stream, csv, ret) != (size_t)ret) {
					pdo_pgsqlsw_error_msg(dbh, PGRES_FATAL_ERROR, "Unable to write to file");
					PQfreemem(csv);
					php_stream_close(stream);
					PDO_HANDLE_DBH_ERR();
					RETURN_FALSE;
...
static PHP_METHOD(PDO, pgsqlLOBOpen)
{
	...
	if (lfd >= 0) {
		php_stream *stream = (php_stream *) pdo_pgsqlsw_create_lob_stream(getThis(), lfd, oid);
		if (stream) {
...
static const zend_function_entry *pdo_pgsqlsw_get_driver_methods(pdo_dbh_t *dbh, int kind)
{
	switch (kind) {
...
static int pdo_pgsqlsw_set_attr(pdo_dbh_t *dbh, zend_long attr, zval *val)
{
...
static const struct pdo_dbh_methods pgsql_methods = {
	pgsqlsw_handle_closer,
	pgsqlsw_handle_preparer,
	pgsqlsw_handle_doer,
	pgsqlsw_handle_quoter,
	pgsqlsw_handle_begin,
	pgsqlsw_handle_commit,
	pgsqlsw_handle_rollback,
	pdo_pgsqlsw_set_attr,
	pdo_pgsqlsw_last_insert_id,
	pdo_pgsqlsw_fetch_error_func,
	pdo_pgsqlsw_get_attribute,
	pdo_pgsqlsw_check_liveness,	/* check_liveness */
	pdo_pgsqlsw_get_driver_methods,  /* get_driver_methods */
	NULL,
	pgsqlsw_handle_in_transaction,
};

static int pdo_pgsqlsw_handle_factory(pdo_dbh_t *dbh, zval *driver_options) /* {{{ */
{
	pdo_pgsqlsw_db_handle *H;
	int ret = 0;
	char *conn_str, *p, *e;
	...
	H = (pdo_pgsqlsw_db_handle *) pecalloc(1, sizeof(pdo_pgsqlsw_db_handle), dbh->is_persistent);
	dbh->driver_data = H;
	...
	tmp_user = _pdo_pgsqlsw_escape_credentials(dbh->username);
	tmp_pass = _pdo_pgsqlsw_escape_credentials(dbh->password);
	...
	if (PQstatus(H->server) != CONNECTION_OK) {
		pdo_pgsqlsw_error(dbh, PGRES_FATAL_ERROR, PHP_PDO_PGSQLSW_CONNECTION_FAILURE_SQLSTATE);
		goto cleanup;
	}

	PQsetNoticeProcessor(H->server, (void(*)(void*,const char*))_pdo_pgsqlsw_notice, (void *)&dbh);
...
const pdo_driver_t pdo_pgsqlsw_driver = {
	PDO_DRIVER_HEADER(pgsql),
	pdo_pgsqlsw_handle_factory
};
