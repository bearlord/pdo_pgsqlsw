...
static int pgsqlsw_stmt_dtor(pdo_stmt_t *stmt)
{
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt*)stmt->driver_data;
...
static int pgsqlsw_stmt_execute(pdo_stmt_t *stmt)
{
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt*)stmt->driver_data;
	pdo_pgsqlsw_db_handle *H = S->H;
	...
	if (S->cursor_name) {
		...
		if (status != PGRES_COMMAND_OK && status != PGRES_TUPLES_OK) {
			pdo_pgsqlsw_error_stmt(stmt, status, pdo_pgsqlsw_sqlstate(S->result));
			return 0;
		}
		PQclear(S->result);
...
stmt_retry:
			...
			switch (status) {
				...
				case PGRES_TUPLES_OK:
					...
					break;
				default: {
					char *sqlstate = pdo_pgsqlsw_sqlstate(S->result);
					...
					if (sqlstate && !strcmp(sqlstate, "42P05")) {
						...
						pdo_pgsqlsw_error_stmt(stmt, status, sqlstate);
						return 0;
					}
				}
			}
	...
	if (!stmt->executed && (!stmt->column_count || S->cols == NULL)) {
		...
		S->cols = (pdo_pgsqlsw_column *) ecalloc(stmt->column_count, sizeof(pdo_pgsqlsw_column));
	}
...
static int pgsqlsw_stmt_param_hook(pdo_stmt_t *stmt, struct pdo_bound_param_data *param,
	...
	if (stmt->supports_placeholders == PDO_PLACEHOLDER_NAMED && param->is_param) {
		switch (event_type) {
			...
			case PDO_PARAM_EVT_NORMALIZE:
				...
				if (param->name) {
					if (ZSTR_VAL(param->name)[0] == '$') {
						...
						if (stmt->bound_param_map && (namevar = (char *)zend_hash_find_ptr(stmt->bound_param_map,
								param->name)) != NULL) {
			...
			case PDO_PARAM_EVT_EXEC_PRE:
				...
				if (!S->param_values) {
					S->param_values = (char **) ecalloc(
							zend_hash_num_elements(stmt->bound_param_map),
							sizeof(char*));
					S->param_lengths = (int *) ecalloc(
							zend_hash_num_elements(stmt->bound_param_map),
							sizeof(int));
					S->param_formats = (int *) ecalloc(
							zend_hash_num_elements(stmt->bound_param_map),
							sizeof(int));
					S->param_types = (Oid *) ecalloc(
							zend_hash_num_elements(stmt->bound_param_map),
							sizeof(Oid));
				}
				if (param->paramno >= 0) {
					...
					*/
					...
					if (PDO_PARAM_TYPE(param->param_type) == PDO_PARAM_LOB &&
						...
						if (stm) {
							if (php_stream_is(stm, &pdo_pgsqlsw_lob_stream_ops)) {
								struct pdo_pgsqlsw_lob_self *self = (struct pdo_pgsqlsw_lob_self*)stm->abstract;
								pdo_pgsqlsw_bound_param *P = (pdo_pgsqlsw_bound_param *) param->driver_data;

								if (P == NULL) {
									P = (pdo_pgsqlsw_bound_param *) ecalloc(1, sizeof(*P));
									param->driver_data = P;
								}
							...
							pdo_pgsqlsw_error_stmt(stmt, PGRES_FATAL_ERROR, "HY105");
							return 0;
						}
					}
...
static int pgsqlsw_stmt_fetch(pdo_stmt_t *stmt,
...
static int pgsqlsw_stmt_describe(pdo_stmt_t *stmt, int colno)
{
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt*)stmt->driver_data;
	...
	switch (S->cols[colno].pgsql_type) {
		...
		case OIDOID:
			...
			if (stmt->bound_columns && (
					(param = (pdo_bound_param_data *) zend_hash_index_find_ptr(stmt->bound_columns, colno)) != NULL ||
					(param = (pdo_bound_param_data *) zend_hash_find_ptr(stmt->bound_columns, cols[colno].name)) != NULL)) {
...
static int pgsqlsw_stmt_get_col(pdo_stmt_t *stmt, int colno, char **ptr, size_t *len, int *caller_frees )
{
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt*)stmt->driver_data;
	struct pdo_column_data *cols = stmt->columns;
	size_t tmp_len;
	...
	/* We have already increased count by 1 in pgsqlsw_stmt_fetch() */
		...
		switch (cols[colno].param_type) {
			...
			case PDO_PARAM_LOB:
				if (S->cols[colno].pgsql_type == OIDOID) {
					...
					if (loid >= 0) {
						*ptr = (char*)pdo_pgsqlsw_create_lob_stream(&stmt->database_object_handle, loid, oid);
						*len = 0;
						return *ptr ? 1 : 0;
					}
					*ptr = NULL;
...
static zend_always_inline char * pdo_pgsqlsw_translate_oid_to_table(Oid oid, PGconn *conn)
{
...
static int pgsqlsw_stmt_get_column_meta(pdo_stmt_t *stmt, zend_long colno, zval *return_value)
{
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt*)stmt->driver_data;
	PGresult *res;
	char *q=NULL;
	...
	table_name = pdo_pgsqlsw_translate_oid_to_table(table_oid, S->H->server);
	if (table_name) {
...
static int pdo_pgsqlsw_stmt_cursor_closer(pdo_stmt_t *stmt)
{
	pdo_pgsqlsw_stmt *S = (pdo_pgsqlsw_stmt*)stmt->driver_data;
...
const struct pdo_stmt_methods pgsql_stmt_methods = {
	...
	pgsqlsw_stmt_fetch,
	pgsqlsw_stmt_describe,
	pgsqlsw_stmt_get_col,
	pgsqlsw_stmt_param_hook,
	NULL, /* set_attr */
	NULL, /* get_attr */
	pgsqlsw_stmt_get_column_meta,
	NULL,  /* next_rowset */
	pdo_pgsqlsw_stmt_cursor_closer
};
